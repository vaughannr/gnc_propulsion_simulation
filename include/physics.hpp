...
inline Vector3 computeAcceleration(const Vector3 &force, double mass) {
...
// - The object has a mass (required to be positive), a position, and a
// velocity.
...
class PhysicsBody {
public:
  // Constructor requires a positive mass; initial position and velocity are
  // optional.
  PhysicsBody(double mass, double initialPosition = 0.0,
              double initialVelocity = 0.0)
      : mass_(mass), position_(initialPosition), velocity_(initialVelocity),
  ...
  void applyForce(double force) { accumulatedForce_ += force; }

  // Resets the accumulated force to zero.
  void resetForce() { accumulatedForce_ = 0.0; }
...
// it can work with any standard container (e.g., std::vector) of PhysicsBody
// objects.
class PhysicsEngine {
public:
