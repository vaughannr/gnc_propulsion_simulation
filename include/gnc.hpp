...
// target position and the current position. A proportional gain scales the
// error.
...
class ControlModule {
public:
  ControlModule(double kp = 1.0) : kp_(kp) {}

  // Returns the control acceleration:
  // acceleration = kp * (desiredVelocity - currentVelocity)
  double computeControlAcceleration(double desiredVelocity,
...
//   new position = current position + current velocity * dt + 0.5 *
//   acceleration * dt^2
...
// GNC System
// ------------------------------
// The GNCSystem class combines the three modules into one unit. Given an
// initial state (position and velocity) and a target position, its update()
// method uses:
...
public:
  ...
  GNCSystem(double initialPosition, double initialVelocity, double target)
      : state_{initialPosition, initialVelocity}, guidance_(0.1, target),
        control_(1.0), latestControlAcceleration_(0.0) {}
