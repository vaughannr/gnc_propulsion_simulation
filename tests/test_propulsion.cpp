...
TEST(PropulsionSystemTest, FuelExhaustion) {
    // Choose a fuel mass that is too low to sustain the selected throttle for dt=1 s.
    ...
    // but we only have 0.1 kg. Thus, update() should adjust the thrust based on available fuel.
    engine.update(1.0);
    ...
    // The achievable thrust is computed as: remainingFuel / (dt * consumptionRate)
...
// class ThrusterEfficiencyTest : public ::testing::TestWithParam<std::tuple<double, double, double>> {};
...
// INSTANTIATE_TEST_CASE_P(ThrusterTests, ThrusterEfficiencyTest, testing::Values(
