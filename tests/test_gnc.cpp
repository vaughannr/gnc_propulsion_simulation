...
// Test the GuidanceModule: with kp = 0.2 and target = 100.0 and a current
// position of 50, the computed desired velocity should be 0.2*(100-50) = 10.0.
...
// Test the ControlModule: with kp = 2.0, a desired velocity of 15.0, and a
// current velocity of 5.0, the control acceleration should be 2.0*(15-5)
// = 20.0.
TEST(ControlModuleTest, ComputeControlAcceleration) {
  ...
  double acceleration =
      control.computeControlAcceleration(desiredVelocity, currentState);
  EXPECT_DOUBLE_EQ(acceleration, 20.0);
}

// Test the NavigationModule: ensure that for a positive dt, the state is
// updated using kinematic equations. For an initial state of {position = 0,
// velocity = 0}, an acceleration of 10, and dt = 2, we expect: position = 0 +
// 0*2 + 0.5*10*(4)=20 and velocity = 0 + 10*2 = 20.
...
// If dt = 0, then the state should not change even though the control
...
// We simulate a sequence of updates with a small time step, and after many
// iterations, the position should be near the target and the velocity near
// zero.
TEST(GNCSystemTest, ConvergenceToTarget) {
  ...
  State state = gnc.getCurrentState();
  // Because of the proportional laws used, the system should be near 100 with
  // negligible velocity.
