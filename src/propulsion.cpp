...
PropulsionSystem::PropulsionSystem(double fuelMass, double maxThrust)
    : fuelMass_(fuelMass), maxThrust_(maxThrust), currentThrust_(0.0),
      running_(false),
...
double PropulsionSystem::getFuelMass() const { return fuelMass_; }

double PropulsionSystem::getCurrentThrust() const { return currentThrust_; }

bool PropulsionSystem::isRunning() const { return running_; }
